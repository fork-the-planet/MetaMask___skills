...
> **MetaMask note (verified) — current offenders:**
> - **lodash:** 98 files import from the main `'lodash'` package; only 5 use submodule paths; `babel-plugin-lodash` is **not** installed. Prefer `import x from 'lodash/x'` (or native equivalents). Biggest easy bundle win after barrel exports.
> - **Date libraries:** both **`dayjs` and `luxon`** are installed (4 + 6 files) — overlapping responsibilities. Consolidate onto one.
> - **moment.js:** not present (good). Keep it that way.
> - Evaluate every new dependency with the steps below **before** adding; pair with [bundle-analyze-js.md](bundle-analyze-js.md) to confirm real impact.
