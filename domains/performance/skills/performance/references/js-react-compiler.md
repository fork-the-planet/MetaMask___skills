...
> **MetaMask note:** React Compiler is **already installed and configured**. It runs on an opt-in path list in `babel.config.js` (`target:'18'`, currently `app/components/Nav` + `app/components/UI/DeepLinkModal`). For the MetaMask opt-in workflow (healthcheck, plugin ordering, `yarn watch:clean`), see **[mm-react-compiler.md](mm-react-compiler.md)** — that is the canonical file for this repo. The content below is the upstream reference for how the compiler works.
...
> **MetaMask: setup is DONE — skip Steps 2–4.** The plugin, runtime, ESLint plugin, and babel config already exist. Do **not** re-install or re-configure. The only step you run is the **healthcheck** (Step 1) before opting a directory into `pathsToInclude`. For the MetaMask opt-in workflow use [mm-react-compiler.md](mm-react-compiler.md). The snippets below are set to **`target: '18'`** to match this repo (upstream Callstack defaults to `'19'`).
...
const ReactCompilerConfig = {
  target: '18', // this repo targets '18' (use '19' only on React 19 / RN ≥ 0.78 if you opt to)
};
