...
- Generated harness state must be clean before harness install. Remove stale ignored outputs (`temp/agentic/recipes`, `.agent/recipe-harness/extension`) or install to a task-local `temp/tasks/<run>/harness/recipes` path, then run recipes from that path. When using a task-local install, pass the same path to harness live/verify with `--out <task-local-recipes>`; do not let live verify fall back to stale/default `temp/agentic/recipes`. Do not use `--force` to overwrite ambiguous local modifications.
